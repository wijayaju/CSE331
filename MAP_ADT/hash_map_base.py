...
class HashMapBase(MapBase, ABC):
  ...
  """
    ...
    """
        ...
        self._capacity = cap
        self._table = [None] * self._capacity  # Initialize table
        self._n = 0  # number of entries in the map
        self._prime = p  # prime for MAD compression
        self._scale = 1 + randrange(p - 1)  # scale from 1 to p-1 for MAD
        self._shift = randrange(p)  # shift from 0 to p-1 for MAD

    # THE GOOD HASH FUNCTION, USES MAD METHOD, UNCOMMENT AFTER DEMO!!!
    # def _hash_function(self, k):
    #     return (hash(k) * self._scale + self._shift) % self._prime % len(self._table)

    # FOR TESTING TO FORCE COLLISION, COMMENT IT OUT AFTER DEMO

    def _hash_function(self, k):
        return hash(k) % 5  # Force collisions by limiting buckets to 5
